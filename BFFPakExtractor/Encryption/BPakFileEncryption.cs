...
namespace BFFPakExtractor.Encryption
{
    ...
    {
		public static void DecryptData(eEncryptionType encryptionType, Span<byte> data, int len, int keyIndex)
        {
			...
			else if (encryptionType == eEncryptionType.TwoFish)
            {
				...
				throw new NotImplementedException("Twofish encryption is not implemented yet");
            }
		...
		public static void DecryptTwoFish(byte[] data, Span<byte> key)
        {
			...
			int i;
			// 2 by 2
			for (i = 0; i + 1 < tmpKey.Length; i += 2)
			{
				byte tmp1 = (byte)(xor[tIndex++] ^ tmpKey[i]);
				tIndex = Utils.ClampWrap(tIndex, 0, 2);

				byte tmp2 = (byte)(xor[tIndex++] ^ tmpKey[i + 1]);
				tIndex = Utils.ClampWrap(tIndex, 0, 2);
		...
		private static void DecryptRC4(Span<byte> data, int index)
        {
			var key = GetEncKey(index); // Get a copy
